...
class IngredientInterval {
    ...
    setExact(ingredient: Ingredient) {
        this.elements.push(ingredient);
        this.min = ingredient.exactPercent;
        this.max = ingredient.exactPercent;
        this.exact = true;
    }

    setMax(m: number) { this.max = m; }
    setMin(m: number) { this.min = m; }
...
function analyse(message: string) {
    ...
    let intervalList: IngredientInterval[] = [];

    let allIngredients: Ingredient[] = [];
    let latestIngredient: Ingredient = new Ingredient('');
    let latestInterval: IngredientInterval = new IngredientInterval();
    latestInterval.max = 100;
    ...
    for (let part of parts) {
        ...
        if (inBracket) {
            ...
            if (nameAndPercent.percent) {
                ...
                if (latestInterval.elements.length > 0) {
                    intervalList.push(latestInterval);
                    latestInterval = new IngredientInterval();
                }
                if (intervalList.length > 0){
                    intervalList[intervalList.length-1].min = p;
                }
                latestInterval.setExact(latestIngredient);
                intervalList.push(latestInterval);
                latestInterval = new IngredientInterval();
                latestInterval.max = p;
            } else {
